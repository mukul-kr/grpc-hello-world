...
class GreetServicer(greet_pb2_grpc.GreeterServicer):
    ...
    def ParrotSaysHello(self, request, context):
        ...
        for i in range(3):
            hello_reply = greet_pb2.HelloReply()
            hello_reply.message = f"{request.greeting} {request.name} {i}"
            yield hello_reply
            time.sleep(1)
    ...
    def ChattyClientSaysHello(self, request_iterator, context):
        ...
        delayed_reply.message = f"You have sent {len(delayed_reply.request)} requests. Please wait for a while."
        return delayed_reply
            # delayed_reply.message = f"{request.
